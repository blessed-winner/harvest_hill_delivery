...
export function ClientNegotiations() {
  ...
  // Active negotiation drawer state
  ...
  const fetchClientNegotiations = async () => {
    try {
      ...
      // Filter exclusively for Client B2B Negotiations
  ...
  const handleSendCounterOffer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setActiveThread(res);
      setCounterMessage('');
      toast("Counter-offer sent to client successfully!", "success");
      fetchClientNegotiations();
    } catch (err: any) {
  ...
  const handleDeleteThread = async (threadId: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      toast("Negotiation thread deleted.", "success");
      fetchClientNegotiations();
    } catch (err: any) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-white p-4 rounded-xl border border-outline-variant/40 shadow-2xs">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Search by client name, email, company, or produce..."
            value={searchQuery}
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-on-surface-variant block font-medium">{buyer.company || buyer.email}</span>
                          </div>
                        </div>
                      </td>
